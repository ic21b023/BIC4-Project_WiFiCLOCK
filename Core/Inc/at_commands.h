...
/*
#def/*ine CMD_AT_CIPSEND_CLIENT(length)				AT_CIPSEND AT_SET TOSTR(length) CRLF /* Delete/create a TCP/SSL server */
/*
...
#define RESP_AT_SEND_OK								"SEND OK"
 ...
 inline char * get_set_or_get(char * str){
  char set[]="=,?";
  return  ((strlen(str)-1)-strcspn ( str, set)) > 0 ? AT_SET : AT_GET;
}

 inline char * get_command(char * str){
    char set[]="=,?";
  return  strtok ( str, set);
}
